...
import { formatDay, formatRange, lateSignUpText, organizerNames, SAVE_ERROR } from './parentText'

// One event open for sign-up: a toggle per eligible child. After the deadline
// the toggles are locked and clicking one says whom to write to.
...
const lateNotice = ref('')

function click({ member }) {
  if (ended.value) {
    lateNotice.value = lateSignUpText(member.nickname || member.firstName, props.organizers[0])
  } else {
    emit('toggle', member)
  }
}
    ...
    <article :aria-label="event.title">
      ...
      >
        ...
        <button
          ...
          :disabled="readOnly || child.saving"
      ...
      <p aria-live="polite" class="m-0 empty:hidden">
        <span v-if="lateNotice" class="note-warm mt-3 block">{{ lateNotice }}</span>
      </p>
